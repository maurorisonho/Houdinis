...
class CircuitOptimizer:
    ...
    def optimize_circuit(self, circuit: Dict[str, Any]) -> Tuple[Dict[str, Any], CircuitMetrics]:
        """
        ...
        metrics = CircuitMetrics(
            ...
            gate_reduction=(original_gates - len(optimized_gates)) / original_gates if original_gates > 0 else 0,
        ...
        logger.info(f"Optimization: {original_gates} → {len(optimized_gates)} gates ({metrics.gate_reduction*100:.1f}% reduction)")
        ...
        while i < len(gates):
            if i + 1 < len(gates):
                ...
                if (gate1.get('type') == gate2.get('type') and
                    ...
                    self._are_inverse_gates(gate1, gate2)):
                    i += 2  # Skip both gates
                    continue
    ...
    def _merge_rotation_gates(self, gates: List[Dict]) -> List[Dict]:
        ...
        while i < len(gates):
            ...
            if gate.get('type') in ['RX', 'RY', 'RZ']:
                ...
                while j < len(gates):
                    next_gate = gates[j]
                    if (next_gate.get('type') == gate_type and
                        next_gate.get('qubit') == qubit):
