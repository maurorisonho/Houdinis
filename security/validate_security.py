    ...
    def test_input_validation(self) -> None:
        ...
        for host in valid_hosts:
            if SecurityConfig.validate_hostname(host):
                self.log_result(
                    f"Hostname Validation: {host}", "PASS", "Valid hostname accepted"
                )
            else:
                self.log_result(
                    f"Hostname Validation: {host}", "FAIL", "Valid hostname rejected"
                )

        for host in invalid_hosts:
            if not SecurityConfig.validate_hostname(host):
                self.log_result(
                    f"Hostname Validation: {host}", "PASS", "Invalid hostname rejected"
                )
            else:
                self.log_result(
                    f"Hostname Validation: {host}", "FAIL", "Invalid hostname accepted"
                )
        ...
        for port in valid_ports:
            if SecurityConfig.validate_port(port):
                self.log_result(
                    f"Port Validation: {port}", "PASS", "Valid port accepted"
                )
            else:
                self.log_result(
                    f"Port Validation: {port}", "FAIL", "Valid port rejected"
                )

        for port in invalid_ports:
            if not SecurityConfig.validate_port(port):
                self.log_result(
                    f"Port Validation: {port}", "PASS", "Invalid port rejected"
                )
            else:
                self.log_result(
    ...
    def test_command_validation(self) -> None:
        ...
        for cmd in safe_commands:
            if SecurityConfig.validate_command(cmd):
                self.log_result(
                    f"Command Validation: {cmd}", "PASS", "Safe command accepted"
                )
            else:
                self.log_result(
                    f"Command Validation: {cmd}", "FAIL", "Safe command rejected"
                )

        for cmd in dangerous_commands:
            if not SecurityConfig.validate_command(cmd):
                self.log_result(
                    f"Command Validation: {cmd}", "PASS", "Dangerous command rejected"
                )
            else:
                self.log_result(
    ...
    def test_secure_file_operations(self) -> None:
        ...
        for path in valid_paths:
            if secure_files.validate_path(path, allow_create=True):
                self.log_result(
                    f"Path Validation: {path}", "PASS", "Valid path accepted"
                )
            else:
                self.log_result(
                    f"Path Validation: {path}", "FAIL", "Valid path rejected"
                )

        for path in invalid_paths:
            if not secure_files.validate_path(path, allow_create=True):
                self.log_result(
                    f"Path Validation: {path}", "PASS", "Invalid path rejected"
                )
            else:
                self.log_result(
                    f"Path Validation: {path}", "FAIL", "Invalid path accepted"
                )
        ...
        if temp_file:
            if secure_files.secure_write_file(temp_file, test_content):
                self.log_result(
                    "Secure File Write", "PASS", "File written successfully"
                )
                ...
                if read_content == test_content:
                    self.log_result(
                        "Secure File Read", "PASS", "File read successfully"
                    )
                else:
                    self.log_result("Secure File Read", "FAIL", "File content mismatch")

                # Test file permissions
                file_stat = os.stat(temp_file)
                if file_stat.st_mode & 0o077 == 0:  # Only owner can access
                    self.log_result(
                        "File Permissions", "PASS", "Secure permissions set"
                    )
                else:
                    self.log_result("File Permissions", "FAIL", "Insecure permissions")

                # Cleanup
                secure_files.secure_delete_file(temp_file)
            else:
                self.log_result("Secure File Write", "FAIL", "Failed to write file")
    ...
    def test_dependency_security(self) -> None:
        ...
        try:
            ...
            result = subprocess.run(
                ["pip", "show", "safety"], capture_output=True, text=True
            )
            ...
            if result.returncode == 0:
                ...
                if safety_result.returncode == 0:
                    self.log_result(
                        "Dependency Security", "PASS", "No known vulnerabilities found"
                    )
                else:
                    self.log_result(
                        "Dependency Security", "WARN", "Potential vulnerabilities found"
                    )
            else:
                self.log_result(
                    "Dependency Security",
