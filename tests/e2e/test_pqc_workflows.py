...
class TestKyberAttackWorkflow:
    ...
    def test_complete_kyber_security_audit(self):
        ...
        for level in security_levels:
            ...
            assert 'vulnerable' in timing_result, "Timing attack missing vulnerability field"
            print(f"[E2E-PQC]   Timing attack: {'VULNERABLE' if timing_result['vulnerable'] else 'SAFE'}")
            
            # Power analysis
            power_result = attacker.power_analysis_attack(num_traces=300)
            assert 'vulnerable' in power_result, "Power attack missing vulnerability field"
            print(f"[E2E-PQC]   Power analysis: {'VULNERABLE' if power_result['vulnerable'] else 'SAFE'}")
            
            # Cache timing
            cache_result = attacker.cache_timing_attack(num_measurements=500)
            assert 'vulnerable' in cache_result, "Cache attack missing vulnerability field"
            print(f"[E2E-PQC]   Cache timing: {'VULNERABLE' if cache_result['vulnerable'] else 'SAFE'}")
            ...
            print(f"[E2E-PQC]   CCA: {'VULNERABLE' if cca_result['vulnerable'] else 'SAFE'}")
            ...
            assert 'vulnerable' in lattice_result, "Lattice attack missing vulnerability field"
            print(f"[E2E-PQC]   Lattice: {'VULNERABLE' if lattice_result['vulnerable'] else 'SAFE'}")
            ...
            required_fields = [
                'parameter_set', 'timestamp', 'timing_attack', 
                'power_analysis', 'cca_attack', 'fault_injection',
                'cache_timing', 'lattice_reduction', 'parameter_analysis',
                'overall_assessment'
            ]
...
class TestDilithiumAttackWorkflow:
    ...
    def test_complete_dilithium_signature_analysis(self):
        ...
        for level in security_levels:
            ...
            print(f"[E2E-PQC]   Forgery: {'VULNERABLE' if forgery_result['vulnerable'] else 'SAFE'}")
            ...
            print(f"[E2E-PQC]   Nonce reuse: {'VULNERABLE' if nonce_result['vulnerable'] else 'SAFE'}")
            ...
            print(f"[E2E-PQC]   Timing: {'VULNERABLE' if timing_result['vulnerable'] else 'SAFE'}")
            ...
            print(f"[E2E-PQC]   Hash: {'VULNERABLE' if hash_result['vulnerable'] else 'SAFE'}")
            ...
            required_fields = [
                'parameter_set', 'timestamp', 'signature_forgery',
                'nonce_reuse', 'timing_attack', 'fault_injection',
                'hash_collision', 'lattice_attack', 'parameter_analysis',
                'overall_assessment'
            ]
...
class TestFALCONSPHINCSWorkflow:
    ...
    def test_falcon_complete_analysis(self):
        ...
        for param_set in ['falcon512', 'falcon1024']:
            ...
            print(f"[E2E-PQC]   NTRU lattice: {'VULNERABLE' if lattice_result['vulnerable'] else 'SAFE'}")
            ...
            print(f"[E2E-PQC]   Gaussian: {'VULNERABLE' if gaussian_result['vulnerable'] else 'SAFE'}")
    ...
    def test_sphincs_complete_analysis(self):
        ...
        for param_set in ['sphincs128f', 'sphincs256f']:
            ...
            print(f"[E2E-PQC]   Hash collision: {'VULNERABLE' if collision_result['vulnerable'] else 'SAFE'}")
            
            # Multi-target attack
            multi_result = attacker.multi_target_attack(num_targets=10, attempts_per_target=1000)
            assert 'vulnerable' in multi_result, "Multi-target result incomplete"
            print(f"[E2E-PQC]   Multi-target: {'VULNERABLE' if multi_result['vulnerable'] else 'SAFE'}")
            ...
            print(f"[E2E-PQC]   Avg entropy: {size_result['average_entropy']:.2f} bits/byte")
...
class TestHybridPQCWorkflow:
    ...
    def test_complete_hybrid_security_assessment(self):
        ...
        for config in configurations:
            ...
            print(f"[E2E-PQC]   Downgrade: {'VULNERABLE' if downgrade_result['vulnerable'] else 'SAFE'}")
            ...
            print(f"[E2E-PQC]   KEX confusion: {'VULNERABLE' if kex_result['vulnerable'] else 'SAFE'}")
            ...
            print(f"[E2E-PQC]   Sig bypass: {'VULNERABLE' if sig_result['vulnerable'] else 'SAFE'}")
            ...
            print(f"[E2E-PQC]   Transition: {'VULNERABLE' if transition_result['vulnerable'] else 'SAFE'}")
            ...
            # Verify audit completeness
            required_fields = [
                'configuration', 'timestamp', 'downgrade_attack',
                'key_exchange_confusion', 'signature_verification_bypass',
                'protocol_state_confusion', 'transition_period_exploit',
                'hybrid_kdf_weakness', 'implementation_inconsistency',
                'overall_assessment'
            ]
    ...
    def test_hybrid_deployment_scenarios(self):
        ...
        if 'deployment_phases' in result:
            ...
            for phase, data in phases.items():
                print(f"[E2E-PQC]   {phase}: {data.get('vulnerable_scenarios', 0)} vulnerable scenarios")
...
'''
            ...
            critical_count = sum(1 for a in assessments if a.threat_level == 'immediate')
            ...
            required_report_fields = [
                'summary', 'vulnerabilities', 'recommendations',
                'migration_paths', 'timeline', 'budget_estimate'
            ]
            ...
            print(f"[E2E-PQC]   Total budget: ${report['budget_estimate']['min']}-${report['budget_estimate']['max']}")
