...
class TestRSAAttackWorkflow:
    ...
    def test_rsa_attack_with_error_handling(self):
        ...
        if factors:
            assert (
                1 in factors or N_prime in factors
            ), "Prime factorization should be trivial"
...
class TestGroverAttackWorkflow:
    ...
    def test_complete_grover_search_workflow(self):
        ...
        if success:
            ...
            ), f"Wrong key found: {found_key} ≠ {target_key}"
    ...
    def test_grover_multiple_targets(self):
        ...
        for target in target_keys:
            result = grover.search(key_space_bits=key_space_bits, target_key=target)
...
class TestMultiBackendWorkflow:
    ...
    def test_parallel_quantum_operations(self):
        ...
        assert (
            len(results) == num_circuits
        ), f"Expected {num_circuits} results, got {len(results)}"
...
class TestRealWorldScenarios:
    ...
    def test_password_recovery_scenario(self):
        ...
        result = grover.search(key_space_bits=pin_bits, target_key=target_pin)
...
def run_e2e_tests():
    ...
    # Run with pytest
    pytest.main([__file__, "-v", "-m", "e2e", "--tb=short", "--durations=10"])
