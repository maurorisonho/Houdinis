...
def demo_algorithm_comparison():
    ...
    print("-" * 90)
    print(
        f"{'Algorithm':<20} {'Backend':<18} {'Time (s)':<10} {'Qubits':<8} {'Success Rate':<12}"
    )
    print("-" * 90)
    ...
    for algorithm in algorithms:
        for i, backend in enumerate(backends):
            ...
            print(
                f"{alg_name:<20} {backend:<18} {data['time']:<10.1f} {data['qubits']:<8} {data['success_rate']:<12.2%}"
            )
        print("-" * 90)
    ...
    for algorithm in algorithms:
        best_backend = min(
            performance_data[algorithm].items(), key=lambda x: x[1]["time"]
        )
...
def main():
    ...
    try:
        ...
        print("\nNext steps:")
        print(
            "  1. Install quantum computing dependencies: pip install -r requirements.txt"
        )
